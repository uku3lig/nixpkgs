...
in
{
  ...
  # TODO docs and release notes (including in commit)
  options.services.reposilite = {
    enable = lib.mkEnableOption "Reposilite";
    package = lib.mkPackageOption pkgs "reposilite" {
      example = "reposilite.override { plugins = with reposilitePlugins; [checksum groovy]; }";
    };
    ...
    extraArgs = lib.mkOption {
      ...
      '';
      default = "";
      example = "--token name:tempsecrettoken";
    };
      ...
      script =
        lib.optionalString (cfg.settings.database == null) (
          if cfg.database.passwordFile == null then
            ...
            ''
              export REPOSILITE_LOCAL_DATABASE="${dbString} $(<${cfg.database.passwordFile})"
            ''
        )
        ...
        ''
        + ''
          ${lib.getExe cfg.package} --local-configuration ${configFile} --local-configuration-mode none --working-directory ${cfg.workingDirectory} ${cfg.extraArgs}
        '';
